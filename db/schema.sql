...
-- Кожна сторінка зберігається як JSON-blob у полі `data`, окремий рядок на
-- кожну локаль. Композитний PK (slug, locale); відсутній переклад → fallback
-- на 'en' у застосунку. Структура JSON валідується через Zod-схеми.
-- ============================================================
